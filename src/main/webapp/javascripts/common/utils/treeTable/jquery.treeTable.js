...
(function ($) {
    ...
    $.fn.treeTable = function (opts) {
        ...
        if ($('head').find('#tree_table_' + opts.theme).length == 0) {
            $('head').append('<link id="tree_table_' + opts.theme + '" href="../../javascripts/common/utils/treeTable/vsStyle/jquery.treeTable.css" rel="stylesheet" type="text/css" />');
        }
        ...
        $treeTable.click(function (event) {
            ...
            if ($target.attr('arrow')) {
                ...
                if (className == css['AN'] + ' ' + css['LO'] || className == css['AN'] + ' ' + css['O']) {
                    var id = $target.parents('tr[haschild]')[0].id;
                    $target.attr('class', css['AN'] + " " + (className.indexOf(css['O']) != -1 ?  css['S'] : css['LS']));

                    //关闭所有孩子的tr
                    shut(id);
					return;
                } else if (className == css['AN'] + ' ' + css['LS'] || className == css['AN'] + ' ' + css['S']) {
                    var id = $target.parents('tr')[0].id;
                    $target.attr('class', css['AN'] + " " + (className.indexOf(css['S']) != -1 ? css['O'] : css['LO']));
        ...
        function initRelation($trs, hideLevel) {
            ...
            $trs.each(function (i) {
                ...
                //判断是否要隐藏限制的层次
                if (hideLevel) {
                    depth > opts.expandLevel && $tr.css({'display':'none'});
