    ...
    .directive('uiLayout', ['$parse', function ($parse) {
        ...
        return {
            ...
            compile: function compile(tElement, tAttrs) {
                ...
                var maxCount = 0;
                for (_i = 0; _i < _child_len; ++_i) {
                    if (angular.element(_childens[_i]).attr('min-size')) {
                        maxCount++
                    }
                    if (angular.element(_childens[_i]).attr('max-size')) {
                        minCount++
                    }
                }
                ...
                if (maxCount > 0) {
                    for (_i = 0; _i < _child_len; ++_i) {
                        ...
                        // - the global size on the layout option
                        // - 'auto' Fair separation of the remaining space
                        opts.sizes[_i] = angular.element(_childens[_i]).attr('min-size') || opts.sizes[_i] || 'auto';
                    }
                    maxSize = convertNumericDataTypesToPencents(opts.sizes, tElement[0]['offset' + (isUsingColumnFlow ? 'Width' : 'Height')]);
                } else {
                    maxSize = [100, 0];
                }
                ...
                if (minCount > 0) {
                    for (_i = 0; _i < _child_len; ++_i) {
                        ...
                        // - the global size on the layout option
                        // - 'auto' Fair separation of the remaining space
                        opts.sizes[_i] = angular.element(_childens[_i]).attr('max-size') || opts.sizes[_i] || 'auto';
                    }
                    minSize = convertNumericDataTypesToPencents(opts.sizes, tElement[0]['offset' + (isUsingColumnFlow ? 'Width' : 'Height')]);
                } else {
                    minSize = [0, 100];
                }
                ...
                if (_child_len > 1) {
                    ...
                    for (_i = 0; _i < _child_len; ++_i) {
                        ...
                        if (_i < _child_len - 1) {
                            ...
                            bar.attr("maxsize", maxSize[0]);
                            bar.attr("minsize", minSize[0]);
                            area.after(bar);
                        }
                    }
                }
    ...
    .directive('uiSplitbar', function () {
        ...
        return {
            ...
            link: function (scope, iElement, iAttrs, parentLayout) {
                var minWidth = iAttrs.minsize;
                var maxWidth = iAttrs.maxsize;
                ...
                function _draw() {
                    ...
                    if (the_pos < parseInt(minWidth) || the_pos > parseInt(maxWidth)) {
                        return false;
                    }
