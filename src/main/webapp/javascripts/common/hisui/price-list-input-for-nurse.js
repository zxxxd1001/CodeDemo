                ...
                $scope.openMe = function (targetElement) {
                    if (_closeFlag) {
                        ...
                        _template = angular.element(
                            angular.copy($templateCache.get('price-list-for-nurse-bill-template.html'))
                        );
                        ...
                        if ((bodyOffsetHeight - (position.top + position.height)) < 179) {
                            resultTop = position.top - 221;
                        } else {
                            resultTop = position.top + 26;
                        }
                        //护士站单病人  费用  弹出输入法显示不全   做下判断
                        if((screenWidth-position.left) > 800){
                            resultLeft = position.left+1;
                        }else{
                            resultLeft = screenWidth - 800;
                        }
