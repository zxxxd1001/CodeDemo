...
document.write ('    <DIV align=center id=\"softkeyboard\" name=\"softkeyboard\" style=\"position:absolute; left:133px; top:393px; width:517px; z-index:180;display:none\">');
document.write ('  <table width=\"348\" border=\"1\" align=\"center\" cellpadding=\"3\" cellspacing=\"1\" bgcolor=\"#388E8E\">');
document.write ('    <FORM name=Calc action=\"\" method=post autocomplete=\"off\">');
document.write ('      <INPUT type=hidden value=ok name=action2>');
document.write ('      <tr> ');
document.write ('        <td align=\"right\" bgcolor=\"#FF9900\" align=\"center\"> ');
document.write ('          <INPUT class=button type=button value=输入完毕 name=\"Submit3\" onclick=\"OverInput(curEditName);\"> <INPUT class=button type=reset value=输错重来 name=\"Submit23\"  onclick=\"resetInput(curEditName);\"> ');
...
function addValue(newValue) {
    if (CapsLockValue==0) {
        drugWestForm.drugWestInput.value += newValue;
        drugWestForm.drugWestInput.focus();
    }else {
        drugWestForm.drugWestInput.value += newValue.toUpperCase();
        drugWestForm.drugWestInput.focus();
    }
}
//实现BackSpace键的功能
function setpassvalue() {
    var longnum=drugWestForm.drugWestInput.value.length;
    var num = drugWestForm.drugWestInput.value.substr(0,longnum-1);
    drugWestForm.drugWestInput.value=num;
    drugWestForm.drugWestInput.focus();
}
//输入完毕
...
//定义当前需用软键盘的表单和控件的名称
var curEditName ="drugWestForm.drugWestInput"
