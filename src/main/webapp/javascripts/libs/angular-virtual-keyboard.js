...
var VKI = function(customConfig, layout, deadKeys, keyInputCallback) {
  ...
  this.VKI_position = function(force) {
    if (!self.VKI_relative) {
      ...
      self.VKI_keyboard.style.top = (config.topPosition === undefined) ? '0px' : config.topPosition;
      self.VKI_keyboard.style.left = (config.leftPosition === undefined) ? '0px': config.leftPosition;
      self.VKI_keyboard.style.width = '50%';
      self.VKI_keyboard.style.height = '115px';
      return;
    }
    if (self.VKI_target) {
...
.directive('ngVirtualKeyboard', ['ngVirtualKeyboardService', '$timeout', '$injector',
	...
	return {
		...
		link: function(scope, elements, attrs, ngModelCtrl) {
			if(!ngModelCtrl||getMachineConfig("Soft_Keyboard")!=1){
				return;
			}
